...
def _exec_search_docs_catalog(args: dict) -> str:
    ...
    if not catalog:
        return json.dumps({
            "status": "ok", "result": [],
            "llm_text": f"【目录检索】未找到与 '{query}' 匹配的文档。建议尝试更宽泛的关键词或指定 source 参数。"
        }, ensure_ascii=False)
...
def execute_tool_call(tool_name: str, arguments_str, user_cfg=None, frontend_cfg=None) -> str:
    ...
    try:
        ...
        if tool_name in _ATOMIC_TOOLS:
            ...
            if name == "query_jira_metadata":
                return fn(args, user_pat=user_pat)
            return fn(args)
...
# Alice V2.0 系统提示词
CORE_SYSTEM_PROMPT_V2 = """你是 Alice V2.0，一个精通 Jira 项目和知识库管理的 AI 助手。

## 核心能力
你可以使用以下工具来帮助用户:

1. **query_jira_metadata(issue_key)** — 获取 Jira 任务的元数据（标题、状态、经办人）
2. **get_issue_commits(issue_key)** — 获取任务关联的 SVN 代码提交和变更记录
3. **search_docs_catalog(query, source)** — 在知识库中搜索匹配的文档目录（仅返回标题+摘要）
4. **read_specific_doc(doc_id, source)** — 读取指定文档的全文内容

## 重要工作流程

### 知识库检索 (LlamaIndex 层级检索模式):
1. 先用 search_docs_catalog 搜索目录，获取候选文档列表
2. 根据标题和摘要选择最相关的文档
3. 再用 read_specific_doc 读取选定文档的全文
4. 基于全文内容回答用户问题

### 代码变更分析:
1. 先用 query_jira_metadata 获取任务基本信息
2. 再用 get_issue_commits 获取代码变更
3. 分析 Diff 内容，总结变更逻辑

## 规则
- 始终基于工具返回的事实数据回答，不要编造
- 如果工具返回空结果，诚实告知用户
- 回答使用中文，结构化输出（表格、列表）
- 严禁在输出中使用任何 XML/DSML 标签"""
    ...
    def generate_stream():
        ...
        while step < max_steps:
            ...
            if finish_reason == "tool_calls" and msg.get("tool_calls"):
                ...
                # 使用 asyncio + to_thread 实现并发
                async def _execute_concurrent():
                    tasks = [asyncio.to_thread(_run_one_tool, tc) for tc in tcs]
                    return await asyncio.gather(*tasks)

                try:
                    results = asyncio.run(_execute_concurrent())
                except Exception as e:
                    logger.error(f"[ReAct] Concurrent execution failed: {e}")
                    # 回退串行
                    results = [_run_one_tool(tc) for tc in tcs]
