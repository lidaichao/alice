<template>
  <div class="page-wrap">
    <ConfigCard title="Notion 知识库" :editing="s.editLock.notion">
      ...
      <template #actions>
        <el-button size="small" :loading="s.testing.notion" :disabled="s.editLock.notion" @click="s.testNotionSystem">
          测试并拉取数据库
        </el-button>
        <el-button v-if="!s.editLock.notion" size="small" type="primary" plain @click="s.startEdit('notion')">
          编辑配置
        </el-button>
      </template>
        ...
        <el-col :span="12">
          <el-form-item label="Notion API Key">
            <el-input
              ...
              :class="{ 'readonly-field': !s.editLock.notion }"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
    ...
    <ConfigCard title="Google 云盘知识库" :editing="s.editLock.gdrive">
      ...
      <template #actions>
        <el-button size="small" :loading="s.testing.gdrive" :disabled="s.editLock.gdrive" @click="s.testGDriveSystem">
          测试并列出文件夹内容
        </el-button>
        <el-button v-if="!s.editLock.gdrive" size="small" type="primary" plain @click="s.startEdit('gdrive')">
          编辑配置
        </el-button>
      </template>
      ...
      <el-form-item label="Google API Key (Service Account)">
        <el-input
          ...
          :class="{ 'readonly-field': !s.editLock.gdrive }"
        />
      </el-form-item>
      <el-row :gutter="16">
        <el-col :span="12">
          <el-form-item label="局域网代理 IP">
            <el-input v-model="s.state.kb.GDRIVE_PROXY_IP" :readonly="!s.editLock.gdrive" :class="{ 'readonly-field': !s.editLock.gdrive }" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="代理端口">
            <el-input v-model="s.state.kb.GDRIVE_PROXY_PORT" :readonly="!s.editLock.gdrive" :class="{ 'readonly-field': !s.editLock.gdrive }" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item label="指定检索的 Folders">
        ...
        />
        <p v-else-if="!s.gdriveFoldersList.length" class="section-hint">尚未配置文件夹</p>
      </el-form-item>
...
function pickNotionDb(row) {
  ...
  ElMessage.success('已选用数据库 ID，请记得保存配置');
}
</script>
