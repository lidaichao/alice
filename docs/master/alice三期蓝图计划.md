...
|| v3.1-rc33 | 2026-06-12 | **Spec 合规修复（LangGraph §七 + n8n §一/§五）**：agent/state.py L5 import 路径+L11 Sequence[BaseMessage] · agent_graph.py L306 去 hasattr · ai_bridge.py L2964+L3959 去 hasattr+去 webhook 多余 X-N8N-API-KEY · setup_n8n.py httpHeaderAuth→jiraSoftwareApi · n8n_webhook_workflow.json +responseMode · pytest 120/0/3 ✅ |
|| v3.1-rc32 | 2026-06-12 | **深钻 CTO 审计修复**：agent_graph 死代码合并 + 错误码 N8N_UNREACHABLE + 模型 fallback 同步 composer-2.5 + chatSlice confirm_card 类型补全 buildConfirmCardFromApi + test_n8n_bridge 断言同步 · AL-155 已规划中 · pytest 120/0/3 ✅ |
|| v3.1-rc30 | 2026-06-12 | **深钻波次3 QA回归验证 (AL-160~161 + AL-144)**：<br>├ AL-160 淬火产物回归检查（空状态/Onboarding/快捷卡片/审批面板/设置中心/引擎选择器 6项逐项验证）· AL-161 Ask模式行为验证（3条知识问题确认无tool_calls）· npm build 双前端 ✅ · pytest 120/0/3 ✅ · 全子任务+父任务 AL-144 流转→完成 |
|| v3.1-rc29 | 2026-06-12 | **深钻波次1 Agent管道接通+深度模型默认启用 (AL-146~159)**：<br>├ AL-146 前端Auto/Agent→/v1/agent/stream路由切换 · AL-147 intent_router jira_write→jira_create_issue · AL-148 Agent SSE UI适配(type message/confirm_card/done/error) · AL-156 默认模型 composer-2.5 · AL-157 引擎选择器显示模型名 · AL-158 depth思考文案 · AL-159 首次深度模型轻提示 · 规则: +子任务自身description必读 · +epic三层正向读取 · npm build 双前端 ✅ · pytest 120/0/3 ✅ · 全部流转→完成 |
|| v3.1-rc30 | 2026-06-12 | **深钻波次2：Jira写入闭环 (AL-149~155 · Epic AL-140)**：<br>├ AL-149 jira_create_issue工具 · AL-150 ConfirmCard已修复 · AL-151 端到端联调 · AL-152 6种error_code规范化 · AL-153 中文降级文案 · AL-154 审批自动刷新 · AL-155 npm build+pytest 120/0/3 ✅ · 🔗 AL-142~155 |
|| v3.1-rc34 | 2026-06-13 | **火星计划 · 第一波：源码+配置部署到服务器 (AL-167~177 · Epic AL-166)**：<br>**P1 环境验证**：<br>├ ✅ AL-168 硬件：Ubuntu 26.04 · 8核 · 30GB · 200G+916G+229G 磁盘<br>├ ✅ AL-169 Docker 29.1.3 + Compose 2.40.3 已安装<br>├ ✅ AL-170 Git 2.53.0 已安装<br>├ ✅ AL-171 网络：百度200 · Jira 200 · GitHub不通 · Docker Hub不通<br>├ ✅ AL-172 代理方案确认：GitHub不通→scp替代git clone<br>**P2 源码+配置部署**：<br>├ ✅ AL-174 scp同步 13,980文件/37MB → /home/alice/alice<br>├ ✅ AL-175 .env.prod 创建（Jira/Dify/n8n/DeepSeek 凭据完整填入）<br>├ ✅ AL-176 双前端构建：Chat(1.4s) + Admin-UI(4.5s) ✅<br>├ ✅ AL-177 docker-compose.prod.yml config 语法校验通过<br>**已知阻塞**：Docker Hub 不通 → 镜像无法拉取 → 需协调者配置 registry mirror · npm build 双前端 ✅ · 🔗 AL-167~177 · 🔗 Epic AL-166 |
|| v3.1-rc36 | 2026-06-13 | **火星计划 · 第二波：Docker 镜像加速 + 8 容器启动 + 三轮验证通过 (AL-178~183 · Epic AL-166)**：<br>**第〇步 镜像加速**：<br>├ ✅ DaoCloud 镜像 `docker.m.daocloud.io` 配置生效（中科大/网易 DNS 不通）<br>├ ✅ `docker pull hello-world` 成功<br>**AL-179 Docker Compose 启动**：<br>├ ✅ `docker compose -f docker-compose.prod.yml up -d --build`（8/8 容器全部 running）<br>├ ✅ alice-hub (healthy) · dify-api · dify-db (healthy) · dify-redis · dify-weaviate · dify-sandbox · n8n · admin-ui<br>**AL-180 全栈健康检查**：<br>├ ✅ Alice Hub /health 200 · Dify /health 200 · n8n /healthz 200 · Admin /index.html 200<br>├ ✅ Agent SSE 冒烟：POST /v1/agent/stream → message 事件正常<br>**AL-181 setup_n8n**：<br>├ ⚠️ n8n API key 401 —— docker-compose 缺 N8N_API_KEY env var（需 P4 补充）<br>├ ⚠️ 凭据+工作流导入未完（P4 修复后重跑 setup_n8n.py 即可）<br>**AL-182 check_dify**：<br>├ ✅ Dify v1.3.0 健康 · App Key ✅ · Dataset Key ✅<br>├ ⚠️ RAG 检索 HTTP 500 —— 知识库初次启动无文档入库（预期，P4 上传知识文档）<br>**AL-183 浏览器截图**：<br>├ ✅ Chat 前端 `http://192.168.72.147:5000/chat/` 登录页正常<br>├ ✅ Admin 后台 `http://192.168.72.147:8080/` 正常<br>**修复**：docker-compose.prod.yml +frontend/dist 卷挂载 · 🔗 AL-178~183 · 🔗 Epic AL-166 |
|| v3.1-rc37 | 2026-06-13 | **火星计划 · P4+P5：数据迁移修复 + 全队 localhost→192.168.72.147 同步完成 (AL-184~195 · Epic AL-166)**：<br>**P4 数据迁移修复**：<br>├ ✅ n8n N8N_API_KEY 注入 docker-compose.prod.yml（已重启 n8n 容器）<br>├ ⚠️ setup_n8n 仍 401 —— n8n 1.87.0 CLI 无 `user-management:auth-external-access-token` 命令，需兔子提供正确 API key<br>├ ✅ .env.prod `chmod 600` 权限加固<br>├ ✅ Dify Web `http://192.168.72.147:5001/health` 200 可访问（等待协调者上传知识文档）<br>├ ✅ check_dify：Dify v1.3.0 健康 · 双 Key 格式正确 · RAG 500（无文档，预期）<br>├ ✅ n8n Webhook 外网可达：`http://192.168.72.147:5678/webhook/alice-hub-callback` HTTP 404（工作流未注册，端点正确）<br>├ ✅ global_config.json 对比无差异（JIRA_BASE_URL/PAT/DEEPSEEK 等 6 字段一致）<br>├ ✅ AL-189 账号迁移跳过（服务器首次部署无历史）<br>**P5 规则同步**：<br>├ ✅ AL-191 RABBIT_MEMORY.md 42 处替换<br>├ ✅ AL-192 RABBIT_ROADMAP.md 无需替换（全在存档段）<br>├ ✅ AL-193 CURRENT_SPRINT.md 2 处替换<br>├ ✅ AL-194 rg 全量扫描：剩余匹配仅存档段/元文本/Docker内网/本地代理<br>├ ✅ AL-195 TEAM_CHANNEL.md 变更通知发布<br>**已知待办**：n8n API key 认证问题 → 需兔子 SSH 进容器用 n8n 1.87.0 正确 CLI 生成 key · Dify 知识文档待协调者上传 · 🔗 AL-184~195 · 🔗 Epic AL-166 |
