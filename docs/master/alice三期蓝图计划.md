...
| v3.0-phase3 | 2026-06-10 | **v3.0 Phase 3 Admin 统一代理层 + P1/P3 修正（兔子下发）**：<br>**P1 修正**：<br>├ ☑ n8n API Key 从容器 SQLite 完整读取（原 `.env.n8n` 截断为 `eyJh...`）→ curl 验证 200 OK<br>**P3 修正**：<br>├ ☑ `jira_create.json` 新增 Save Original Function 节点（保存原始 Webhook payload 到 `$json._original_payload`）<br>├ ☑ Jira Create 节点全部参数引用改为 `$json._original_payload.*`（project_key/summary/issue_type/description/idempotency_key）<br>├ ☑ Idempotency Check JQL 引用改为 `$json._original_payload.idempotency_key`<br>**Admin 代理层**：<br>├ ☑ `ai_bridge.py` 新增 N8N 配置加载（读 `.env.n8n` · N8N_BASE_URL/N8N_API_KEY/N8N_WEBHOOK_URL）<br>├ ☑ `POST /v1/admin/proxy/dify/rag`（DifyRagProxyRequest Pydantic · timeout=10s · 4类中文错误翻译）<br>├ ☑ `POST /v1/admin/proxy/n8n/jira`（N8nJiraProxyRequest Pydantic · timeout=3s · 4类中文错误翻译）<br>├ ☑ Admin 前端无需改动（已通过 Hub 代理调用，无直调 Dify/n8n 的 fetch）<br>**新增文件清单**：无<br>**修改文件清单**：`.env.n8n`（完整 Key）、`n8n_workflows/jira_create.json`（Save Original + 引用修复）、`ai_bridge.py`（+N8N 配置 + 2 个代理端点 + Pydantic 请求模型） |
